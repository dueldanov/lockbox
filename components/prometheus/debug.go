...
func configureDebug() {
	...
	deps.SnapshotManager.Events.SnapshotMetricsUpdated.Hook(func(metrics *snapshot.Metrics) {
	...
	deps.PruningManager.Events.PruningMetricsUpdated.Hook(func(metrics *pruning.Metrics) {
	...
	deps.Tangle.Events.ConfirmationMetricsUpdated.Hook(func(metrics *whiteflag.ConfirmationMetrics) {
